...
class STEAM_OT_use_shading_nodes(Operator):
    """Enable nodes on a material, world or light"""
    bl_idname = "steam.use_shading_nodes"
    bl_label = "Use Nodes"
...
class STEAM_OT_add_aov(bpy.types.Operator):
    """Add an AOV pass"""
    bl_idname="steam.add_aov"
    bl_label="Add AOV"
    ...
    def execute(self, context):
        ...
        steam_view_layer.aovs.add()

        view_layer.update_render_passes()
        return {'FINISHED'}


class STEAM_OT_remove_aov(bpy.types.Operator):
    """Remove an AOV pass"""
    bl_idname="steam.remove_aov"
    bl_label="Remove AOV"
    ...
    def execute(self, context):
        ...
        steam_view_layer.aovs.remove(steam_view_layer.active_aov)

        view_layer.update_render_passes()
        return {'FINISHED'}


class STEAM_OT_denoise_animation(Operator):
    "Denoise rendered animation sequence using current scene and view " \
    "layer settings. Requires denoising data passes and output to " \
    "OpenEXR multilayer files"
    bl_idname = "steam.denoise_animation"
    ...
    def execute(self, context):
        ...
        from . import _steam
        try:
            _steam.denoise(preferences.as_pointer(),
                            scene.as_pointer(),
...
class STEAM_OT_merge_images(Operator):
    ...
    def execute(self, context):
        ...
        from . import _steam
        try:
            _steam.merge(input=in_filepaths, output=out_filepath)
        except Exception as e:
            self.report({'ERROR'}, str(e))
            return {'FINISHED'}
...
classes = (
    STEAM_OT_use_shading_nodes,
    STEAM_OT_add_aov,
    STEAM_OT_remove_aov,
    STEAM_OT_denoise_animation,
    STEAM_OT_merge_images
)
